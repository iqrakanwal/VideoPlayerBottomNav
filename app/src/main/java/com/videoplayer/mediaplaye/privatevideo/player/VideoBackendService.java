...
import com.videoplayer.mediaplaye.privatevideo.player.EqualizerPlayer.EqualizedExoPlayer;
import com.videoplayer.mediaplaye.privatevideo.player.activities.PlayVideoActivity;
import com.videoplayer.mediaplaye.privatevideo.player.activities.PlayerManager;
...
public class VideoBackendService extends Service implements AudioManager.OnAudioFocusChangeListener {

    PreferencesUtility preferencesUtility;
    private PlayerManager playerManager;
    //private SimpleExoPlayer simpleExoPlayer;
    private serviceCallBack serviceCallbacks;
    private FileItem currentVideoPlaying = new FileItem();
    ...
    public void change(PlaybackParameters param) {
        playerManager.getSimpleExoPlayer().setPlaybackParameters(param);
    }

    public void session() {

        playerManager.sessionset();
    }
    ...
    public void onCreate() {
        ...
        mExoPlayer = new EqualizedExoPlayer(getApplicationContext(), playerManager.getSimpleExoPlayer());
        ...
        playerManager.getSimpleExoPlayer().addListener(new Player.EventListener() {
            ...
            public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
                if (playbackState == Player.STATE_ENDED) {
                    if (mMainutils.videoItemsPlaylist.size() == 0 || getNextPosition() == -1)
                        return;
                    if (getNextPosition() >= mMainutils.videoItemsPlaylist.size())
                        return;
                    SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
                    boolean name = sharedPreferences.getBoolean("autoplay", false);
                    if (name) {
                        mMainutils.playingVideo = mMainutils.videoItemsPlaylist.get(getNextPosition());
                        playVideo(0, false);
                    }
            ...
            public void onPlaybackParametersChanged(PlaybackParameters playbackParameters) {
                //  playerManager.setplaybackspeed();
            }

            @Override
            public void onSeekProcessed() {
                Mainutils.getInstance().isSeekBarProcessing = false;
            }
        });
    ...
    private void addPopupView() {
        ...
        btnFullScreenMode.setOnClickListener(v -> {
            ...
            removePopup();
            Intent intent = new Intent(VideoBackendService.this, PlayVideoActivity.class);
    ...
    private void addbootomLayout() {
        removePopupView();
        bottomcontainer = new FrameLayout(this);
        bottomcontainer.setBackgroundColor(Color.BLACK);
        LayoutInflater inflater = (LayoutInflater) getApplicationContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View view = inflater.inflate(R.layout.popup_video, bottomcontainer);
        playerView = view.findViewById(R.id.player_view);
        playerView.setPlayer(playerManager.getCurrentPlayer());
        layout_drag = view.findViewById(R.id.layout_all_control_container);
        relativeLayout = view.findViewById(R.id.layout_control_top);
    ...
    public class VideoBinder extends Binder {
        public VideoBackendService getService() {
            return VideoBackendService.this;
        }
    }

    private Notification buildNotification() {
        if (artWork == null) {
            loadImage(mMainutils.playingVideo);
            return null;
        }
        if (Mainutils.getInstance().playingVideo == null) return null;
        FileItem videoItem = Mainutils.getInstance().playingVideo;
        ...
        Intent nowPlayingIntent = new Intent(this, PlayVideoActivity.class);
        ...
        androidx.core.app.NotificationCompat.Builder builder = new androidx.core.app.NotificationCompat.Builder(this, NOTIFICATIONCHANNELID)
                ...
                .setContentTitle(videoItem.getFileTitle())
    ...
    private void loadImage(FileItem videoItem) {
        Glide.with(this).asBitmap()
                .load(videoItem.getPath())
                .listener(new RequestListener<Bitmap>() {
                              @Override
                              public boolean onLoadFailed(@Nullable GlideException e, Object o, Target<Bitmap> target, boolean b) {
                                  Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.musicicon);
...
//        playerManager.gsetPlaybackParams(param);
//    }
    ...
    private void setUpMediaSession() {
        ...
        mSession.setCallback(new MediaSessionCompat.Callback() {
            ...
            public boolean onMediaButtonEvent(Intent mediaButtonEvent) {
                return MediaButtonIntentReceiver.handleIntent(VideoBackendService.this, mediaButtonEvent);
            }
        });
    ...
    private void prev() {
        FileItem videoItem = getPreviousVideo();
        if (videoItem == null) return;
        mMainutils.playingVideo = videoItem;
        playVideo(0, false);
    }

    private FileItem getPreviousVideo() {
        int prPos = getPrePosition();
        if (mMainutils.videoItemsPlaylist.size() == 0 || prPos == -1) return null;
        return mMainutils.videoItemsPlaylist.get(prPos);
    }
    ...
    private int getPrePosition() {
        ...
        if (playerManager.getCurrentPlayer().getShuffleModeEnabled()) {
            ...
            if (previousPosition < 0) previousPosition = mMainutils.videoItemsPlaylist.size() - 1;
    ...
    public int getCurrentPosition() {
        ...
        if (mMainutils.videoItemsPlaylist.size() == 0) return -1;
        for (int i = 0; i < mMainutils.videoItemsPlaylist.size(); i++) {
            if (mMainutils.playingVideo.getPath().equals(mMainutils.videoItemsPlaylist.get(i).getPath())) {
                currentPosition = i;
            }
        }
    ...
    private void gotoNext() {
        ...
        mMainutils.playingVideo = getNextVideo();
        playVideo(0, false);
    }
    ...
    private FileItem getNextVideo() {
        if (mMainutils.videoItemsPlaylist.size() == 0 || getForceNextPosition() <= -1) return null;
        if (getForceNextPosition() >= mMainutils.videoItemsPlaylist.size()) return null;
        return mMainutils.videoItemsPlaylist.get(getForceNextPosition());
    }
    ...
    private int getForceNextPosition() {
        ...
        if (playerManager.getCurrentPlayer().getShuffleModeEnabled()) {
            ...
            if (nextPosition >= mMainutils.videoItemsPlaylist.size()) nextPosition = 0;
    ...
    private int getNextPosition() {
        ...
        if (playerManager.getCurrentPlayer().getShuffleModeEnabled()) {
            ...
            for (int i = 0; i < mMainutils.videoItemsPlaylist.size(); i++) {
                if (mMainutils.playingVideo.getPath().equals(mMainutils.videoItemsPlaylist.get(i).getPath())) {
                    currentPosition = i + 1;
                    if (currentPosition >= mMainutils.videoItemsPlaylist.size())
    ...
    private PendingIntent retrievePlaybackAction(final String action) {
        final ComponentName serviceName = new ComponentName(this, VideoBackendService.class);
    ...
    public void playVideo(long seekPosition, boolean playAsPopup) {
        ...
        if (!currentVideoPlaying.getPath().contains(Mainutils.getInstance().getPath())) {
            currentVideoPlaying = Mainutils.getInstance().playingVideo;

            playerManager.prepare(true, false);
            //if(seekPosition >0) simpleExoPlayer.seekTo(seekPosition);
            playerManager.setPlayWhenReady(Mainutils.getInstance().isPlaying);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (AppsUtils.checkPermission(Manifest.permission.READ_EXTERNAL_STORAGE)) {
                    loadImage(Mainutils.getInstance().playingVideo);
                } else artWork = null;
            } else loadImage(Mainutils.getInstance().playingVideo);
        }
    ...
    public void createShuffleArray() {
        if (mMainutils.videoItemsPlaylist.size() <= 1) return;
        shuffleArray = new int[mMainutils.videoItemsPlaylist.size()];
        for (int i = 0; i < mMainutils.videoItemsPlaylist.size(); i++) {
            shuffleArray[i] = i;
        }
    ...
    private int getCurrentShufflePosition() {
        ...
        if (shuffleArray.length != mMainutils.videoItemsPlaylist.size()) createShuffleArray();
        if (mMainutils.videoItemsPlaylist.size() == 0)
            return -1;
        for (int i = 0; i < mMainutils.videoItemsPlaylist.size(); i++) {
            if (mMainutils.playingVideo.getPath().equals(mMainutils.videoItemsPlaylist.get(i).getPath()))
